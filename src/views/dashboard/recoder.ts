...
class PCMRecorder {
  ...
  */
   ...
   * @return {FormData}
   */
  public getPCMFormData (): FormData {
    if (this.recorder) {
      const name = "vpr.pcm";
      const PCM_MIME_TYPE = "audio/wave; codecs=1";
      let blob: Blob = this.recorder.getPCMBlob();
      blob = new Blob([blob], { type: PCM_MIME_TYPE }); // blob 默认的 type 是 application/octet-stream
